...
description: Coordinate RunPane workstreams from a Pane Session or an orchestrating agent. Covers dispatching work to Pane agents, confirming delivery, handling external text safely, checking PR readiness, and reporting progress. Use when an agent drives one or many Pane workstreams through RunPane.
---
...
`pane-work-prioritizer` with the Pane Chat work-question guide. Those answers
are read-only; start a workstream only when the user authorizes work.

## Where implementation happens

In a Pane Session, the Session keeps discussion, read-only exploration and
investigation, clarification, and ticket work (`discussion`, `investigate`,
`codebase-explorer`, `create-ticket`). Once a ticket is ready and the user
authorizes implementation, dispatch `astra-ticket` through RunPane in a
suitable Pane or tab. Pass the stable Session ID, the Session's goal, context,
decisions, blockers, next action, evidence, and associated Pane and tab IDs.

`astra-ticket` owns its model, planning, implementation, PR, review, QA, and
CI steps. Name the stage and the artifact when you delegate; the pipeline
carries the rest. Leave the Session's selected agent, profile, and tool
configuration as they are while it runs.

The `pane-orchestrator` skill covers Session identity, Pane association, and
the watcher.
...
through `astra-ticket`'s Pane. When a fix changes the head, the PR goes back
