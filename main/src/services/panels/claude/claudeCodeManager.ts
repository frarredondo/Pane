...
export class ClaudeCodeManager extends AbstractCliManager {
  // Interactive mode state tracking
  // Maps panelId -> { resolve, reject, timeout } for waiting on Claude init
  private readyPromises: Map<string, {
    resolve: () => void;
  ...
  /**
   * Check if a Claude process is currently running for a panel
   */
  isProcessRunning(panelId: string): boolean {
    return this.processes.has(panelId);
  }

  /**
   * Wait for Claude to be ready after spawn (init message received)
   * Returns a promise that resolves when Claude sends the init message
   */
  waitForReady(panelId: string, timeoutMs: number = 30000): Promise<void> {
    return new Promise((resolve, reject) => {
      // Clear any existing promise for this panel
      const existing = this.readyPromises.get(panelId);
      if (existing) {
        clearTimeout(existing.timeout);
        existing.reject(new Error('Superseded by new waitForReady call'));
      }

      const timeout = setTimeout(() => {
        this.readyPromises.delete(panelId);
        reject(new Error(`Claude init timeout after ${timeoutMs}ms for panel ${panelId}`));
      }, timeoutMs);

      this.readyPromises.set(panelId, { resolve, reject, timeout });
      console.log(`[ClaudeCodeManager] Waiting for Claude init message for panel ${panelId}`);
    });
  }

  /**
   * Resolve the waitForReady promise for a panel (called when init message is received)
   */
  private resolveReadyPromise(panelId: string): void {
    const promise = this.readyPromises.get(panelId);
    if (promise) {
      clearTimeout(promise.timeout);
      promise.resolve();
      this.readyPromises.delete(panelId);
      console.log(`[ClaudeCodeManager] Claude ready for panel ${panelId}`);
    }
  }

  /**
   * Reject the waitForReady promise for a panel (called on process exit/error)
   */
  private rejectReadyPromise(panelId: string, error: Error): void {
    const promise = this.readyPromises.get(panelId);
    if (promise) {
      clearTimeout(promise.timeout);
  ...
  /**
   * Override onProcessExit to clean up interactive mode state
   */
  protected override onProcessExit(panelId: string, sessionId: string, exitCode: number): void {
    // Clean up any pending ready promise
    this.rejectReadyPromise(panelId, new Error(`Process exited with code ${exitCode}`));

    if (exitCode !== 0) {
      console.warn(`[ClaudeCodeManager] Process ${panelId} exited with code ${exitCode}`);
    } else {
      console.log(`[ClaudeCodeManager] Process ${panelId} exited normally`);
    }
  }
  ...
  protected buildCommandArgs(options: ClaudeSpawnOptions & { mcpConfigPath?: string | null }): string[] {
    const { sessionId, prompt, isResume, permissionMode, model, mcpConfigPath, isInteractive } = options;
    ...
    // In interactive mode, prompt will be sent via stdin after spawn
  ...
  protected parseCliOutput(data: string, panelId: string, sessionId: string): Array<{ panelId: string; sessionId: string; type: 'json' | 'stdout' | 'stderr'; data: unknown; timestamp: Date }> {
    ...
    try {
      ...
      if (jsonMessage.type === 'system' && jsonMessage.subtype === 'init') {
        // INTERACTIVE MODE: Resolve waitForReady promise - Claude is ready for input
        this.resolveReadyPromise(panelId);

        // Note: Claude's session ID is stored automatically via addPanelOutput() in SessionManager
        // when it processes the init message (see sessionManager.ts addPanelOutput)
        if (jsonMessage.session_id) {
          console.log(`[ClaudeCodeManager] Claude session ID detected: ${jsonMessage.session_id} for panel ${panelId}`);
        }
      ...
      // INTERACTIVE MODE: Detect response completion (ready for next input)
      if (jsonMessage.type === 'result') {
        this.emit('ready-for-input', { panelId, sessionId });
        console.log(`[ClaudeCodeManager] Response complete (result message) for panel ${panelId}`);
      }
  ...
  async startPanel(panelId: string, sessionId: string, worktreePath: string, prompt: string, permissionMode?: 'approve' | 'ignore', model?: string): Promise<void> {
    ...
    const useInteractive = config?.useInteractiveMode ?? false;

    if (useInteractive) {
      console.log(`[ClaudeCodeManager] Starting panel ${panelId} in INTERACTIVE mode`);
      // Spawn without -p flag, then send prompt via stdin
      await this.spawnClaudeCode(panelId, sessionId, worktreePath, prompt, undefined, false, permissionMode, model, true);

      // Wait for Claude to be ready (init message)
      try {
        ...
        // stdin write failed or timeout - fall back to headless mode
        console.error(`[ClaudeCodeManager] Interactive mode failed for panel ${panelId}, falling back to headless: ${error}`);
        await this.killProcess(panelId);
        // Retry in headless mode
        return this.spawnClaudeCode(panelId, sessionId, worktreePath, prompt, undefined, false, permissionMode, model, false);
      }
    } else {
      // Existing headless behavior - spawn with -p flag
      return this.spawnClaudeCode(panelId, sessionId, worktreePath, prompt, undefined, false, permissionMode, model, false);
    }
  }
  ...
  async continuePanel(
    ...
    return await withLock(`claude-continue-${panelId}`, async () => {
      ...
      console.log(`[ClaudeCodeManager] continuePanel called for ${panelId} (session ${sessionId}):`, {
        ...
        model,
        useInteractive,
        processRunning: this.isProcessRunning(panelId)
      });

      // INTERACTIVE MODE: If process is still running, send via stdin (no respawn!)
      if (useInteractive && this.isProcessRunning(panelId) && !shouldSkipContinue) {
        ...
        try {
          ...
          // stdin write failed - process may have died, fall through to respawn
          console.error(`[ClaudeCodeManager] stdin write failed for panel ${panelId}: ${error}`);
          // Fall through to respawn with --resume
        }
      }

      // HEADLESS MODE or process not running: Kill and respawn
      if (this.processes.has(panelId)) {
      ...
      if (shouldSkipContinue) {
        ...
        return this.spawnClaudeCode(panelId, sessionId, worktreePath, prompt, [], false, permissionMode, model, useInteractive);
      }

      // For continuing a session, we use the --resume flag
      if (useInteractive) {
        // Interactive mode: spawn with --resume, wait for ready, send prompt via stdin
        console.log(`[ClaudeCodeManager] INTERACTIVE MODE: Respawning with --resume for panel ${panelId}`);
        await this.spawnClaudeCode(panelId, sessionId, worktreePath, prompt, [], true, permissionMode, model, true);
        ...
        // Headless mode: spawn with --resume and -p flag
        console.log(`[ClaudeCodeManager] Using --resume flag for panel ${panelId}`);
        return this.spawnClaudeCode(panelId, sessionId, worktreePath, prompt, [], true, permissionMode, model, false);
      }
    });
  }
  ...
  async spawnClaudeCode(panelId: string, sessionId: string, worktreePath: string, prompt: string, conversationHistory?: string[], isResume: boolean = false, permissionMode?: 'approve' | 'ignore', model?: string, isInteractive: boolean = false): Promise<void> {
