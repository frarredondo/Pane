...
### Fixed
- Pane Chat has its skills again. Pane synced `runpane-orchestrator`, `create-ticket`, `astra-ticket`, `cold-read`, `pane-work-recap`, `pane-work-prioritizer`, and the work-questions guide from the skills repository, which moved them on 2026-08-13; since then every sync failed, and new installs had no `astra-ticket`, `cold-read`, or work-questions guide. These now ship with Pane, rewritten to state what to do, and install into Pane Chat's folder and both project skill roots on every start.
