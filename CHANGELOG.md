...
### Fixed
- Pane Chat has its skills again. Pane synced its agents' skills from the skills repository, which moved them on 2026-08-13; every sync since then failed, and new installs had no `astra-ticket`, `cold-read`, `simple-plan`, `review`, or work-questions guide. All 42 skills Pane Chat uses now ship with Pane, rewritten to say what to do, and install into Pane Chat's folder and both project skill roots on every start. The skill sync is removed, and Pane deletes its old sync folders.
